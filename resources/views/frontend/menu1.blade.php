...
@section('contents')
		...
		<div class="container-main">
			...
			$items=DB::table('restaurent_items')->where('status','Active')->where('catid',$c->id)->get();
			@endphp
			@if(sizeOf($items))
			<div class="row">
				<div class="col-lg-12">
					...
					<p class="primary-color mb-0 try-txt">{{$c->desc}}</p><br>
				</div>
...
@endforeach
		...
		<script type="text/javascript">
			...
			function SelectType(val)
			{
				if(val=='All')
				{
					window.location.href='/restaurents/menu';
				}
				else
				{
					window.location.href='/restaurents/menus/' + val;
				}
			}
